...
#########################################################################################
        for GG in self.graph_list:
            ...
            if isinstance( va[1], adfloat ):
                ...
                self.graphreverse.append(REVERSE_J_LIBRARY2[op](v3, '%s_f' % v3, v1 ,'%s_f' % v1, v2 ))
            
            self.graphreverse.append("    %s=v[%s]" %(v3,self.v_dic[v3]) )
############################################################################################
        ...
        for vv in self.x_list:
            ...
            ret.append(vv.idd) 
            self.graphreverse.append("    %s= self.v[%s]" % (vv.idd,self.v_dic[vv.idd]))
